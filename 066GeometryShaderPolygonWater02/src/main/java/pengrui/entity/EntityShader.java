...
public class EntityShader implements Shaderable{
	...
	public int location_attenuation;
	public int location_texSampler;
	public int location_useSpecular;
	public int location_specularMap;
	...
	public static EntityShader createShader(List<Shaderable> shaders) {
		...
		shader.location_numberOfRows = GL20.glGetUniformLocation(shader.programID, "numberOfRows");
		shader.location_offset = GL20.glGetUniformLocation(shader.programID,"offset");
		shader.location_attenuation = GL20.glGetUniformLocation(shader.programID,"attenuation");
		shader.location_texSampler = GL20.glGetUniformLocation(shader.programID,"texSampler");
		shader.location_useSpecular = GL20.glGetUniformLocation(shader.programID, "useSpecular");
		shader.location_specularMap = GL20.glGetUniformLocation(shader.programID,"specularMap");
		//once uniform
		Matrix4f projectionMatrix = MatrixUtil.createProjectionMatrix();
		GL20.glUseProgram(shader.programID);
		//connect texture unit
		GL20.glUniform1i(shader.location_texSampler,0);
		GL20.glUniform1i(shader.location_specularMap, 1);
		// projection matrix
