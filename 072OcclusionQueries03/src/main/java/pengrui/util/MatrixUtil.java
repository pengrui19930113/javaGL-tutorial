...
//    	mat.m32 = 0;
	...
	public static FloatBuffer calculateMvpMatrixBuffer(Sun sun, Camera camera) {
		mat.setIdentity();
		Vector3f sunPos = sun.getWorldPosition(camera);
		Matrix4f modelMatrix = new Matrix4f();
		Matrix4f.translate(sunPos, modelMatrix, modelMatrix);
		Matrix4f viewMatrix = calculateViewMatrix(camera);
		//remove sun texture rotation
		...
		Matrix4f.scale(new Vector3f(sun.scale,sun.scale,sun.scale), modelViewMatrix, modelViewMatrix);
		Matrix4f.mul(createProjectionMatrix(), modelViewMatrix, mat);
