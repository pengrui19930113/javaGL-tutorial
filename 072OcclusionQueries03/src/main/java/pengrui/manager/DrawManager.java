...
import pengrui.flare.FlareTexture;
...
public class DrawManager {
	...
	public static void draw(EntityShader endityShader,List<Entity> entities,Camera camera,Light light
			...
			, PostProcessing postProcessing, FlareManager flareManager
			) {
		...
		GL11.glViewport(0, 0, postProcessing.sceneFbo.width, postProcessing.sceneFbo.height);
			drawBegin();
				...
				drawSunAndFlare(flareManager,camera);// sun 必须再 skybox 后面渲染
			drawEnd();
	...
	static void drawSunAndFlare(FlareManager flareManager, Camera camera) {
		drawSun(flareManager,camera);
		drawFlare(flareManager,camera);
	}

	static void drawFlare(FlareManager flareManager, Camera camera) {
		if(flareManager.brightness > 0) {
			GL20.glUseProgram(flareManager.flareShader.programID);
			GL30.glBindVertexArray(flareManager.flareVao.vaoid);
			GL20.glEnableVertexAttribArray(0);
			{
				if(GL11.GL_TRUE == GL15.glGetQueryObjecti(flareManager.query.id, GL15.GL_QUERY_RESULT_AVAILABLE)) {
					int visibleSamples = GL15.glGetQueryObjecti(flareManager.query.id, GL15.GL_QUERY_RESULT);
					flareManager.query.inUse = false;
//					System.out.println(visibleSamples);
					flareManager.coverage = visibleSamples/11624f;//8100f
				}
				
				if( null != flareManager.sunScreenNDC 
						&& !flareManager.query.inUse
						//&& GL11.GL_TEXTURE == GL15.glGetQueryObjecti(flareManager.query.id, GL15.GL_QUERY_RESULT_AVAILABLE)
						) {
					GL15.glBeginQuery(flareManager.query.type, flareManager.query.id);
					flareManager.query.inUse = true;
					
					GL11.glEnable(GL11.GL_DEPTH_TEST);
					GL20.glUniform4f(flareManager.flareShader.location_transform, flareManager.sunScreenNDC.x, flareManager.sunScreenNDC.y
							, FlareManager.TEST_QUAD_WIDTH,FlareManager.TEST_QUAD_HEIGHT);
					GL11.glDrawArrays(GL11.GL_TRIANGLE_STRIP, 0, 4);
					
					GL15.glEndQuery(flareManager.query.type);
				}
			}
			...
			GL20.glUniform1f(flareManager.flareShader.location_brightness, flareManager.brightness*flareManager.coverage);
			for(FlareTexture flare:flareManager.flareTextures) {
				GL13.glActiveTexture(GL13.GL_TEXTURE0);
				GL11.glBindTexture(GL11.GL_TEXTURE_2D, flare.texture);
				float xScale = flare.scale;
				float yScale = xScale/(WIDTH/HEIGHT);
				GL20.glUniform4f(flareManager.flareShader.location_transform, flare.xPos, flare.yPos, xScale,yScale);
				GL11.glDrawArrays(GL11.GL_TRIANGLE_STRIP, 0, flareManager.flareVao.vertexCount);
			}
	...
	static void drawSun(FlareManager flareManager, Camera camera) {
		GL11.glEnable(GL11.GL_BLEND);
		GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
		GL20.glUseProgram(flareManager.sunShader.programID);
		GL11.glDepthMask(false);
		GL30.glBindVertexArray(flareManager.sunVao.vaoid);
		GL20.glUniformMatrix4(flareManager.sunShader.location_mvpMatrix, false, MatrixUtil.calculateMvpMatrixBuffer(flareManager.sun,camera));
		GL20.glEnableVertexAttribArray(0);
		GL13.glActiveTexture(GL13.GL_TEXTURE0);
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, flareManager.sun.texture);
		GL11.glDrawArrays(GL11.GL_TRIANGLE_STRIP, 0, flareManager.sunVao.vertexCount);
		GL20.glDisableVertexAttribArray(0);
		GL30.glBindVertexArray(0);
