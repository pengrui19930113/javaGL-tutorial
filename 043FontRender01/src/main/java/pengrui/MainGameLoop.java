...
public class MainGameLoop {
	...
	static final String WINDOW_TITLE = "Hello Font Render";
	...
	public static void main(String[] args) {
		...
		List<StaticText> texts = createTexts(textures);
		...
		while (!Display.isCloseRequested()) {
			...
			ActionManager.action(camera, entities, light, terrain, player,skybox,normalEntities,delta);
...
//			GL11.glPolygonMode(GL11.GL_FRONT_AND_BACK, GL11.GL_LINE);
	...
	static List<StaticText> createTexts(List<Integer> textures) {
		...
		Font font = new Font("font/arial.fnt", TextureLoader.createTexture(textures, "font/arial.png"));
...
//		list.add(new StaticText(0.3f, 0.75f, "0OZabcdefghijklmnopqrstuvwxyz", 1024, font));
		list.add(new StaticText(-1f, -1f, "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz·~!@#$%^&*()_-+=[]{};:'\"\\|,.<>/ ?", 512, font));
		System.out.println(list.get(0).numberOfLines);
		return list;
	}
