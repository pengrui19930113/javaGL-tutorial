...
public class StaticText {
	...
	public StaticText(float xPos,float yPos,String text,int lineMaxPixel,Font font) {
		this(xPos, yPos, text, 0, 0, 0, lineMaxPixel, false,font,5,0.03,1);
	}
...
//						xPixelCurser +=font.fontFile.paddingRight;//指定空格不需要 padding
					}
...
//		int textureRightBottom_x = letter.x+letter.width;
//		int textureRightBottom_y = letter.y+letter.height;
//		
...
//2 有一点被裁剪掉了的感觉
//		int textureLeftTop_x = letter.x+font.fontFile.paddingLeft;
//		int textureLeftTop_y = letter.y+font.fontFile.paddingTop;
//		int textureRightBottom_x = textureLeftTop_x+letter.width-(font.fontFile.paddingLeft+font.fontFile.paddingRight);
//		int textureRightBottom_y = textureLeftTop_y+letter.height-(font.fontFile.paddingTop+font.fontFile.paddingBottom);
//		
//		int textureLeftBottom_x = textureLeftTop_x;
//		int textureLeftBottom_y = textureRightBottom_y;
//		int textureRightTop_x = textureRightBottom_x;
...
//3
		int textureLeftTop_x = letter.x;
		int textureLeftTop_y = letter.y;
		int textureRightBottom_x = textureLeftTop_x+letter.width-(font.fontFile.paddingRight);
...
//		lineMaxPixel TODO
	...
	private class ContextHolder{
		...
		public ContextHolder(List<Line> lines,Line curLine,Word curWord) {
			this.lines = lines;
	...
	private void process(ContextHolder context) {
		List<Line> lines = context.lines;
		 Line curLine = context.curLine;
		 Word curWord = context.curWord;;
		if(curWord.getPixelLength(font) >= lineMaxPixel) {//如果一个单词的长度就大于一行允许的最大像素长度 则补齐当前行的字符为一整行 剩余的拆分为多行
			for(;;) {
				...
				int boundIndex = curWord.characters.size();
				...
				if(boundIndex == curWord.characters.size()-1) {
					...
					if(boundIndex-1 >= 0
							...
							&& SPACE_ASCII != used.characters.get(boundIndex).id) {//如果上一个单词的最后一个字符不是空格 ，换行的时候新增加一个'-'字符 以表示
		...
		context.curLine = curLine;
		context.curWord = curWord;
	}
	
	/**
	 * 该方法与 analyze对比而已(效果一样) 没有使用
	 * @return
	 */
	List<Line> analyze2() {
		...
		for(char c:chars) {
			...
			if(SPACE_ASCII == ascii) {// 表示一个单词已经结束 或者连续多个空格
				if( curWord.characters.isEmpty()
						||SPACE_ASCII == curWord.characters.get(curWord.characters.size()-1).id
						) {//连续多个空格
					curWord.characters.add(curChar);
				}else {//一个单词已经结束
					ContextHolder context = new ContextHolder(lines,curLine,curWord);
					this.process(context);
					curLine = context.curLine;
					curWord = context.curWord;
				}
				continue;
			}else {
				curWord.characters.add(curChar);
		...
		this.process(new ContextHolder(lines,curLine,curWord));
		return lines;
	}
...
//		String test = "oh~ my god , what do you want to do !oh~ my god , what do you want to do !oh~ my god , what do you want to do !oh~ my god , what do you want to do !oh~ my god , what do you want to do !oh~ my god , what do you want to do !";
//		test = " oooooooooooooooooh!                       that is greate!!!!!!                 oooooooooooooooooh!                       that is greate!!!!!!                 oooooooooooooooooh!                       that is greate!!!!!!                 oooooooooooooooooh!                       that is greate!!!!!!                 oooooooooooooooooh!                       that is greate!!!!!!                 oooooooooooooooooh!                       that is greate!!!!!!                 oooooooooooooooooh!                       that is greate!!!!!!                 oooooooooooooooooh!                       that is greate!!!!!!           ~~~~~~~~~~~~     ";
//		byte b[] = {96};
//		test = new String(b);
//		test = "我我我";
//		new StaticText(0, 0, test, 1024, new Font("font/arial.fnt", 0));
//	}
}
