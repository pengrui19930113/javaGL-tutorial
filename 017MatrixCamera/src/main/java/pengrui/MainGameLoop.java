...
public class MainGameLoop {
	...
	static final String WINDOW_TITLE = "Hello Matrix Camera";
	...
	public static void main(String[] args) {
		...
		Terrain terrain = new Terrain(VAO.createTerrainVao(vaos, vbos)
		...
		while (!Display.isCloseRequested()) {
			...
			draw(entityShader,entities,camera,light,terrainShader,terrain,playerShader,player);
	...
	static void draw(EntityShader endityShader,List<Entity> entities,Camera camera,Light light, TerrainShader terrainShader, Terrain terrain, PlayerShader playerShader, Player player) {
		DrawManager.draw(endityShader, entities, camera, light, terrainShader, terrain, playerShader, player);
	}
...
//			GL11.glPolygonMode(GL11.GL_FRONT_AND_BACK, GL11.GL_LINE);
	...
	static List<Entity> createEntities(List<Integer> vaos, List<Integer> vbos, List<Integer> textures){
		...
		VAO shareVao = VAO.createObjVao(vaos, vbos,"models/stanfordBunny.obj");
		int shareTexture = TextureLoader.createTexture(textures,"textures/white.png");
		for(int i=0;i<10;i++) {
