...
public class DrawManager {
	...
	public static void draw(EntityShader endityShader,List<Entity> entities,Camera camera,List<Light> lights
		...
		drawBegin();
		drawTerrain(terrainShader, camera, lights, terrain);
...
//		GL11.glFrontFace(GL11.GL_CW);
	...
	static void drawTerrain(TerrainShader shader,Camera camera,List<Light> lights,Terrain terrain) {
		GL20.glUseProgram(shader.programID);
		...
		for(int i=0;i<Light.MAX_LIGHTS;i++) {
			if(i<lights.size()) {
				...
				GL20.glUniform3f(shader.location_lightPosition[i], light.xPos, light.yPos, light.zPos);
				GL20.glUniform3f(shader.location_lightColour[i], light.red, light.green, light.blue);
			}else {
				GL20.glUniform3f(shader.location_lightPosition[i], 0, 0, 0);
				GL20.glUniform3f(shader.location_lightColour[i], 0, 0, 0);
			}
		}
