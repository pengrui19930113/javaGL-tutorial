...
import static pengrui.CommonConstant.PROGRAM_ATTRIBUTE_NORMALS_INDEX;
import static pengrui.CommonConstant.PROGRAM_ATTRIBUTE_NORMALS_NAME;
...
import static pengrui.CommonConstant.PROGRAM_ATTRIBUTE_TEXTURE_COORDS_NAME;
...
public class TerrainShader implements Shaderable{
	...
	public static TerrainShader createShader(List<Shaderable> shaders) {
		...
		shader.location_lightPosition = new int[Light.MAX_LIGHTS];
		shader.location_lightColour = new int[Light.MAX_LIGHTS];
		for(int i=0;i<Light.MAX_LIGHTS;i++) {
			shader.location_lightPosition[i] = GL20.glGetUniformLocation(shader.programID, "lightPosition["+i+"]");
			shader.location_lightColour[i] = GL20.glGetUniformLocation(shader.programID,"lightColour["+i+"]");
		}
