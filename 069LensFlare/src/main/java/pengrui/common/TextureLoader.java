...
public class TextureLoader {
	...
	private static boolean hasPrintAnisotropicMessage = false;
	
	private static void printAnisotropicMessage() {
		if(!hasPrintAnisotropicMessage) {
			hasPrintAnisotropicMessage = true;
			if(GLContext.getCapabilities().GL_EXT_texture_filter_anisotropic) {
				System.out.println("support anisotropic");
			}else {
				System.out.println("no support anisotropic");
			}
			System.out.println(GL11.glGetFloat(EXTTextureFilterAnisotropic.GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT));
		}
	}
	 ...
	 * 加载的纹理 长宽 必须是 2次幂
	 * 具体代码
	 * {@link org.newdawn.slick.opengl.PNGImageData#loadImage(InputStream, boolean, boolean, int[])}
	 * 查看 89行
	 * @param textures
	 * @param file
	 * @param lodBias
