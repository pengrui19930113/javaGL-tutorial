...
//	public static final Vector2f CENTER_SCREEN = new Vector2f(0.5f,0.5f);
...
//	private static final float[] FLARE_POSITIONS = { -0.5f, -0.5f, -0.5f, 0.5f, 0.5f, -0.5f, 0.5f, 0.5f };//由于 着色器中会进行坐标变换 所以此处的quad的两个三角形是顺时针的
...
//	private static final float[] SUN_POSITIONS = { // 作为渲染四边形 也可以
//			-0.5f,0.5f
//			,-0.5f,-0.5f
//			,0.5f,0.5f
//			,0.5f,-0.5f
//	};
	...
	public FlareManager(List<Integer>vaos,List<Integer>vbos,List<Integer>textures,List<Shaderable> shaders,float spacing) {
...
//			int tex = TextureLoader.createTexture(textures, FLARE_TEXTURE_PATH[i],0);// 使用 createTexture 加载的纹理中心不对称 原因是 由于图片如果不是 2次幂的数值  使用到的库会 对齐 所以使用 createTexture2
	...
	public void update(Camera camera,long delta) {
		...
		Vector2f sunScreenNDC = calculateSunScreenPosition(sunWorldPos,camera);
...
//		float x = (coords.x/coords.w +1)/2;// 透视除法 和坐标变换 到窗口左上角 
//		float y = 1.f-((coords.y/coords.w+1)/2);
