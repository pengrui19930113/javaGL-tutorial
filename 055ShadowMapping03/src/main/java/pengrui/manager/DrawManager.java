...
import pengrui.shadow.ShadowFrameBuffer;
import pengrui.shadow.ShadowShader;
...
public class DrawManager {
	...
	public static void draw(EntityShader endityShader,List<Entity> entities,Camera camera,Light light
			...
			, ShadowShader shadowShader, ShadowFrameBuffer sfb, ShadowBox shadowBox
			) {
	
			drawShadowMap(shadowShader,entities,light,sfb,shadowBox); 
		drawBegin();
			...
			drawTerrain(terrainShader, camera, light, terrain,sfb,shadowBox);
	...
	static void drawShadowMap(ShadowShader shader, List<Entity> entities
			,Light light, ShadowFrameBuffer sfb, ShadowBox shadowBox) {
		//目前只在深度贴图中写入 entity 的深度
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, 0);
		GL30.glBindFramebuffer(GL30.GL_DRAW_FRAMEBUFFER, sfb.fbo);
		GL11.glViewport(0, 0, sfb.width, sfb.height);
		...
		for(Entity entity:entities) {
			...
			if(entity.hasTransparency) {//注意此处是因为 有透明效果 从背面渲染不出来阴影 所以要禁止背面剔除
				GL11.glDisable(GL11.GL_CULL_FACE);
			}
			Matrix4f mvpMatrix = Matrix4f.mul(shadowBox.pvMatrix,MatrixUtil.calculateTransformMatrix(entity), null);
			buffer.clear();
			mvpMatrix.store(buffer);
			buffer.flip();
			GL20.glUniformMatrix4(shader.location_mvpMatrix, false, buffer);
...
//		GL11.glFrontFace(GL11.GL_CW);
	...
	static void drawTerrain(TerrainShader shader,Camera camera,Light light,Terrain terrain
			, ShadowFrameBuffer sfb, ShadowBox shadowBox) {
		GL20.glUseProgram(shader.programID);
		...
		GL20.glUniformMatrix4(shader.location_toShadowMapSpace, false, shadowBox.getToShadwoMapSpaceMatrixBuffer());
		...
		GL11.glBindTexture(GL11.GL_TEXTURE_2D,sfb.shadowMap);
