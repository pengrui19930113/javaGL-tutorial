...
import pengrui.util.OpenglUtil;
/**
 *
 * Shadow
 */
public class MainGameLoop {
	...
	static final String WINDOW_TITLE = "Hello Shadow";
	...
	public static void main(String[] args) {
		...
		ShadowShader shadowShader = ShadowShader.createShader(shaders);
		ShadowFrameBuffer sfb = new ShadowFrameBuffer(textures,fbos,SHADOW_MAP_SIZE,SHADOW_MAP_SIZE);
		ShadowBox shadowBox = new ShadowBox(light);
		...
		List<GuiEntity> guis = createGuis(vaos,vbos,textures,sfb);
		...
		while (!Display.isCloseRequested()) {
			...
			ActionManager.action(camera, entities, light, terrain, player,skybox,normalEntities,texts,fontEffect,shadowBox,delta);
...
//			GL11.glPolygonMode(GL11.GL_FRONT_AND_BACK, GL11.GL_LINE);
	...
	static List<GuiEntity> createGuis(List<Integer> vaos, List<Integer> vbos, List<Integer> textures, ShadowFrameBuffer sfb) {
		GuiEntity.createVao(vaos,vbos);
		List<GuiEntity> list = new LinkedList<GuiEntity>();
		list.add(new GuiEntity(0.8f, -0.8f, 0.2f, 0.2f, sfb.shadowMap));
...
//		list.add(new GuiEntity(-0.9f, -0.9f, 0.1f, 0.1f, TextureLoader.createTexture(textures, "textures/thinmatrix.png")));
	...
	static List<Entity> createEntities(List<Integer> vaos, List<Integer> vbos, List<Integer> textures, Terrain terrain){
		...
		VAO shareVao = ObjVaoLoader.createObjVao(vaos, vbos,"models/tree.obj");
