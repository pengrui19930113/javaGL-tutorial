...
import static pengrui.config.CommonConstant.MAX_HEIGHT;
import static pengrui.config.CommonConstant.MAX_PIXEL_COLOUR;
import static pengrui.config.CommonConstant.TERRAIN_ONE_LINE_SIZE;
import static pengrui.config.CommonConstant.TERRAIN_ONE_LINE_VERTEX_COUNT;
...
public class TerrainVaoLoader {
	...
	public static TerrainHolder createTerrainVao(List<Integer>vaos,List<Integer> vbos,String heightMapPath) {
		...
		TerrainHolder holder = new TerrainHolder();
 ...
 *   					pdown(1)
	...
	private static float getHeight(int x, int z, IHeightGenerator generator) {
		return generator.generateHeight(x, z);
	}
	
	/**
	 * {@link #calculateNormal(int, int, BufferedImage)}
	 * @param x
	 * @param z
	 * @param generator
	 * @return
	 */
	private static Vector3f calculateNormal(int x, int z, IHeightGenerator generator) {
	...
	public static TerrainHolder createTerrainVao(List<Integer>vaos,List<Integer> vbos,IHeightGenerator generator) {
		int VERTEX_COUNT = TERRAIN_ONE_LINE_VERTEX_COUNT;
		float SIZE = TERRAIN_ONE_LINE_SIZE;
		int count = VERTEX_COUNT*VERTEX_COUNT;
		...
		for(int row = 0;row < VERTEX_COUNT; row++) {
			for(int column = 0;column <VERTEX_COUNT; column++) {
				// 顶点坐标y始终是0
				vertices[vertexPointer*3] = column*SIZE / ((float)VERTEX_COUNT-1);
				float height = generator.generateHeight(column,row);
				heights[column][row] = height;
				vertices[vertexPointer*3+1] = height;
				vertices[vertexPointer*3+2] = row*SIZE/ ((float)VERTEX_COUNT-1);
				// 法向量始终 与世界坐标系y轴同向
				Vector3f normal = calculateNormal(column,row,generator);
				normals[vertexPointer*3] = normal.x;
				normals[vertexPointer*3+1] = normal.y;
				normals[vertexPointer*3+2] = normal.z;
