...
public class DrawManager {
	...
	static void drawParticles(ParticleShader shader, Camera camera, Light light,
		...
		for(int textureID:system.particles.keySet()) {
			if(null!=system.particles.get(textureID).get(0) && system.particles.get(textureID).get(0).additive) {
			...
			for(Particle particle:system.particles.get(textureID)) {
				...
				GL20.glUniform2f(shader.location_texOffset1,particle.texOffset1x,particle.texOffset1y);
				GL20.glUniform2f(shader.location_texOffset2,particle.texOffset2x,particle.texOffset2y);
				GL20.glUniform2f(shader.location_texCoordInfo, particle.numberOfRows, particle.blend);
