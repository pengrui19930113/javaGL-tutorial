...
public class EventManager {
	...
	static void doParticles(ParticleSystem system) {
		if(Keyboard.isKeyDown(Keyboard.KEY_Y)) {
			Particle particle = new Particle(5,0,5,0,0,0,0,0.5f,0,1);
			particle.texture = MainGameLoop.fire;
			particle.numberOfRows = 8;
			system.addParticle(particle);
		}
		if(Keyboard.isKeyDown(Keyboard.KEY_U)) {
			Particle particle = new Particle(-5,1,5,0,0,0,0,1f,0,1);
			particle.texture = MainGameLoop.smoke;
			particle.numberOfRows = 8;
			system.addParticle(particle);
		}
