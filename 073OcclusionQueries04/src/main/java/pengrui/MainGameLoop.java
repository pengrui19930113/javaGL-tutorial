...
public class MainGameLoop {
	...
	public static final String WINDOW_TITLE = "Hello Query";
	...
	public static void main(String[] args) {
		...
		FlareManager flareManager = new FlareManager(vaos, vbos, textures, shaders,queries, 0.16f); 
		...
		while (!Display.isCloseRequested()) {
			...
			ActionManager.action(camera, entities, light, terrain, player,skybox,normalEntities,texts,fontEffect,flareManager,delta);
			DrawManager.draw(entityShader, entities, camera, light
					...
					,postProcessing,flareManager
					);
			//control FPS and update
			Display.sync(FPS);
			Display.update();
		}
		destroy(vaos, vbos, shaders, textures,texts,fbos,rbos,queries);
	}
...
//			GL11.glClearColor(SKY_COLOR_RED, SKY_COLOR_GREEN, SKY_COLOR_BLUE, 1);// set background colour;
	...
	static void destroy(List<Integer> vaos, List<Integer> vbos
			...
			, List<StaticText> texts, List<Integer> fbos, List<Integer> rbos, List<Integer> queries) {
