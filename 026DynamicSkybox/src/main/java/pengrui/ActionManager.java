...
public class ActionManager {
	...
	public static void action(Camera camera, List<Entity> entities, Light light, Terrain terrain, Player player, Skybox skybox, long delta) {
//		System.out.println(camera);
//		doEntities(entities,delta);
		
		doCamera(camera,terrain,delta);
		doSkybox(skybox,delta);
	}
	/**
	 * 白天和黑夜的天空图切换
	 * 假设一天中切换时间为2小时
	 * 白天持续时间和晚上持续时间各10小时
	 * time 24000 为一个周期 time=0
	 */
	static void doSkybox(Skybox skybox, long delta) {
		if(skybox.time>=DAY_START && skybox.time<DAY_TO_NIGHT_START_CHANGE){//白天
			skybox.texture1 = skybox.dayTexture;
			skybox.texture2 = skybox.dayTexture;
			skybox.blendFactor = 1;
		}else if(skybox.time>=DAY_TO_NIGHT_START_CHANGE && skybox.time<NIGHT_START){//白天切换到黑夜
			skybox.texture1 = skybox.nightTexture;
			skybox.texture2 = skybox.dayTexture;
			skybox.blendFactor = (NIGHT_START-skybox.time)/CHANGE_TIME;
		}else if(skybox.time>=NIGHT_TO_DAY_START_CHANGE && skybox.time <DAY_START){//黑夜切换到白天
			skybox.texture1 = skybox.dayTexture;
			skybox.texture2 = skybox.nightTexture;
			skybox.blendFactor = (DAY_START-skybox.time)/CHANGE_TIME;
		}else{//黑夜
			skybox.texture1 = skybox.nightTexture;
			skybox.texture2 = skybox.nightTexture;
			skybox.blendFactor = 1;
		}
		skybox.time+=TIME_SPEED*delta;//可以用系统时间 Dispaly.getFrameTimeSeconds() 来计算time
		if(skybox.time>CYCLE_TIME)
			skybox.time%=CYCLE_TIME;
		
		skybox.rotation += SKYBOX_ROTATE_SPEED*delta/1000.f;
		skybox.rotation %= 360.f;
	}
