...
public class MatrixUtil {
	...
	public static final float FOV = 90.f;
...
//		float aspectRatio = (float)WIDTH/(float)HEIGHT;
	...
	static Matrix4f camMat = new Matrix4f();
...
//		Matrix4f.mul(tmp, mat, mat);//先平移 再旋转
	...
	public static Matrix4f calculateViewMatrix(Camera camera) {
    	...
    	Matrix4f.translate(new Vector3f(-camera.xPos, -camera.yPos, -camera.zPos), mat, mat);//translate
    	return mat;
	}
	public static FloatBuffer calculateViewMatrixBuffer(Camera camera) {
...
//    	mat.m32 = 0;
	...
	static FloatBuffer pvBuffer = BufferUtils.createFloatBuffer(16);
	public static FloatBuffer calculateProjectionViewMatrixBuffer(Camera camera) {
		mat.setIdentity();
		Matrix4f.mul(createProjectionMatrix(), calculateViewMatrix(camera), mat);
	...
	static FloatBuffer reflectedEntityBuffer = BufferUtils.createFloatBuffer(16);
	public static FloatBuffer calculateTransformMatrixBuffer(ReflectionEntity entity) {
		mat.setIdentity();
		// 注意此处的 平移旋转缩放变换等价于 =  MT * MR * MS * P (MT:位移矩阵 ，MR:旋转矩阵，MS:缩放矩阵) // 可以查看 caculateTransformMatrix2
		Matrix4f.translate(new Vector3f(entity.xPos, entity.yPos, entity.zPos), mat, mat);//translate
		Matrix4f.rotate((float)Math.toRadians(entity.xRot), X_NORMAL_VECTOR, mat, mat);// rotation
    	Matrix4f.rotate((float)Math.toRadians(entity.yRot), Y_NORMAL_VECTOR, mat, mat);
    	Matrix4f.rotate((float)Math.toRadians(entity.zRot), Z_NORMAL_VECTOR, mat, mat);
    	Matrix4f.scale(new Vector3f(entity.xSca,entity.ySca,entity.zSca), mat, mat); //scale
    	
    	reflectedEntityBuffer.clear();
    	mat.store(reflectedEntityBuffer);
    	reflectedEntityBuffer.flip();
    	return reflectedEntityBuffer;
	}


	static FloatBuffer cubeMapViewBuffer = BufferUtils.createFloatBuffer(16);
	public static FloatBuffer calculateViewMatrixBuffer(CubeMap cubeMap) {
		mat.setIdentity();
		Matrix4f.rotate((float)Math.toRadians(-cubeMap.zRot), Z_NORMAL_VECTOR, mat, mat);
		Matrix4f.rotate((float)Math.toRadians(-cubeMap.xRot), X_NORMAL_VECTOR, mat, mat);// rotation
    	Matrix4f.rotate((float)Math.toRadians(-cubeMap.yRot), Y_NORMAL_VECTOR, mat, mat);
    	Matrix4f.translate(new Vector3f(-cubeMap.xPos, -cubeMap.yPos, -cubeMap.zPos), mat, mat);//translate
		cubeMapViewBuffer.clear();
		mat.store(cubeMapViewBuffer);
		cubeMapViewBuffer.flip();
		return cubeMapViewBuffer;
	}
	static FloatBuffer cubeMapProjectionViewBuffer = BufferUtils.createFloatBuffer(16);
	public static FloatBuffer calculateProjectionViewMatrixBuffer(CubeMap cubeMap) {
		mat.setIdentity();
		Matrix4f.translate(new Vector3f(-cubeMap.xPos, -cubeMap.yPos, -cubeMap.zPos), mat, mat);//translate
		Matrix4f.rotate((float)Math.toRadians(-cubeMap.xRot), X_NORMAL_VECTOR, mat, mat);// rotation
    	Matrix4f.rotate((float)Math.toRadians(-cubeMap.yRot), Y_NORMAL_VECTOR, mat, mat);
    	Matrix4f.rotate((float)Math.toRadians(-cubeMap.zRot), Z_NORMAL_VECTOR, mat, mat);
    	Matrix4f.mul(cubeMap.cubeMapProjectionMatrix, mat, mat);
    	cubeMapProjectionViewBuffer.clear();
		mat.store(cubeMapProjectionViewBuffer);
		cubeMapProjectionViewBuffer.flip();
		return cubeMapProjectionViewBuffer;
	}
