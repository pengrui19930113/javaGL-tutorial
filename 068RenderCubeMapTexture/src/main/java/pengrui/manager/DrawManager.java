...
public class DrawManager {
	...
	public static void draw(EntityShader entityShader,List<Entity> entities,Camera camera,Light light
			...
			, Particle3DShader particle3dShader, Particle3DSystem system
			, PolygonWaterShader polygonWaterShader, PolygonWater water
			, ReflectionEntityShader reflectionEntityShader, List<ReflectionEntity> reflectedEntities
			, CubeMap cubeMap
			) {
		
		FloatBuffer cameraViewMatrixBuffer = MatrixUtil.calculateViewMatrixBuffer(camera);
		FloatBuffer projectionViewMatrixBuffer = MatrixUtil.calculateProjectionViewMatrixBuffer(camera);
		/**
		 * render environment cubemap
		 */
		GL30.glBindFramebuffer(GL30.GL_DRAW_FRAMEBUFFER, cubeMap.fbo);
		GL11.glViewport(0, 0, cubeMap.size, cubeMap.size);
			for(int i=0;i<6;i++) {
				GL30.glFramebufferTexture2D(GL30.GL_FRAMEBUFFER, GL30.GL_COLOR_ATTACHMENT0,
						GL13.GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, cubeMap.texture, 0);
				cubeMap.switchFace(i);
				drawBegin();//
					FloatBuffer cubeMapViewMatrixBuffer = MatrixUtil.calculateViewMatrixBuffer(cubeMap);
//					drawPlayer(playerShader,cubeMapViewMatrixBuffer,light,player);
					drawEntity(entityShader, cubeMapViewMatrixBuffer, light, entities);
					drawSkyBox(skyboxShader,cubeMapViewMatrixBuffer, light,skybox);
				drawEnd();
			}
		/**
		 * render cubemap
		...
		GL11.glViewport(0, 0, postProcessing.sceneFbo.width, postProcessing.sceneFbo.height);
			drawBegin();
//				drawTerrain(terrainShader, cameraViewMatrixBuffer, light, terrain);
//				drawPlayer(playerShader,cameraViewMatrixBuffer,light,player);
				drawEntity(entityShader, cameraViewMatrixBuffer, light, entities);
				drawNormalMappingEntity(normalMappingEntityShader,cameraViewMatrixBuffer,light,normalEntitys);
				drawReflectedEntity(reflectionEntityShader,camera,light,reflectedEntities,cubeMap);
				drawParticles(particle3dShader,projectionViewMatrixBuffer,light,system);
				drawSkyBox(skyboxShader,cameraViewMatrixBuffer, light,skybox);
				drawPolygonWater(polygonWaterShader,projectionViewMatrixBuffer,light,water);
			drawEnd();
	...
	static void drawReflectedEntity(ReflectionEntityShader shader, Camera camera, Light light,
			List<ReflectionEntity> entities, Textureable cubeMap) {
		...
		GL20.glUniform3f(shader.location_cameraPosition, camera.xPos, camera.yPos, camera.zPos);
		...
		for(ReflectionEntity entity:entities) {
			...
			GL20.glUniformMatrix4(shader.location_transformationMatrix, false, MatrixUtil.calculateTransformMatrixBuffer(entity));
	...
	static void drawPolygonWater(PolygonWaterShader shader, FloatBuffer projectionViewMatrixBuffer, Light light,
			PolygonWater water) {
		if(null == water) return;
		GL20.glUseProgram(shader.programID);
		GL20.glUniform1f(shader.location_time, water.time);
		GL20.glUniformMatrix4(shader.location_projectionViewMatrix, false, projectionViewMatrixBuffer);
		GL30.glBindVertexArray(water.vao.vaoid);
		GL20.glEnableVertexAttribArray(0);
		GL11.glDrawElements(GL11.GL_TRIANGLES,water.vao.vertexCount,GL11.GL_UNSIGNED_INT,0);
		GL20.glDisableVertexAttribArray(0);
		GL30.glBindVertexArray(0);
		GL20.glUseProgram(0);
	}

	static void drawParticles(Particle3DShader shader, FloatBuffer projectionViewMatrixBuffer, Light light,
...
//		GL11.glPolygonMode(GL11.GL_FRONT_AND_BACK, GL11.GL_FILL);
	...
	static void drawNormalMappingEntity(NormalMappingEntityShader shader, FloatBuffer viewMatrixBuffer,
		...
		for(NormalMappingEntity entity:normalEntitys) {
			...
			GL20.glUniformMatrix4(shader.location_viewMatrix, false, viewMatrixBuffer);
	...
	public static void drawSkyBox(SkyboxShader shader, FloatBuffer viewMatrixBuffer, Light light, Skybox skybox) {
		GL20.glUseProgram(shader.programID);
		GL20.glUniformMatrix4(shader.location_viewMatrix, false, viewMatrixBuffer);
...
//		GL11.glFrontFace(GL11.GL_CW);
	...
	static void drawTerrain(TerrainShader shader,FloatBuffer viewMatrixBuffer,Light light,Terrain terrain) {
		GL20.glUseProgram(shader.programID);
...
//		GL11.glPolygonMode(GL11.GL_FRONT_AND_BACK, GL11.GL_LINE);
	...
	public static void drawEntity(EntityShader shader,FloatBuffer viewBuffer,Light light,List<Entity> entities) {
	...
	public static void drawPlayer(PlayerShader shader, FloatBuffer viewMatrixBuffer, Light light, Player player) {
		GL11.glEnable(GL11.GL_CULL_FACE);
		GL11.glCullFace(GL11.GL_BACK);
