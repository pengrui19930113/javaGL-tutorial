...
public class ActionManager {
	...
	public static void action(Camera camera, List<Entity> entities, Light light
			...
			, List<StaticText> texts, FontEffect fontEffect, Particle3DSystem system, PolygonWater water, CubeMap environmentCubMap, long delta
			) {
...
//		doEntities(entities,delta);
		...
		doParticleSystem(system,delta);
		doWater(water,delta);
		doCubMap(environmentCubMap,delta);
	}
	
	static void doCubMap(CubeMap environmentCubMap, long delta) {
		if(null == environmentCubMap) return;
		//nothing 暂时只在 0,0,0 的世界坐标系进行渲染
	}

	static void doWater(PolygonWater water, long delta) {
		if(null == water) return;
		water.update(delta);
	}

	static void doParticleSystem(Particle3DSystem system, long delta) {
