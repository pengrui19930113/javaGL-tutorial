...
public class ActionManager {
	...
	public static void action(Camera camera, List<Entity> entities, Light light, Terrain terrain, Player player, Skybox skybox, ParticleSystem system, long delta) {
//		System.out.println(camera);
//		doEntities(entities,delta);
		doParticles(camera,system,player,delta);
		doCamera(camera,terrain,delta);
	}
	
	static void doParticles(Camera camera,ParticleSystem system,Player player, long delta) {
		system.action(camera,delta);
		system.genParticles(player.xPos, player.yPos, player.zPos, delta);
		System.out.println(system.particles.entrySet().iterator().next().getValue().size());
	}
