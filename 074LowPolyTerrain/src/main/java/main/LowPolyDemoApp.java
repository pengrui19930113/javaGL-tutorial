...
import flatTerrain.UnflatTerrainGenerator;
...
public class LowPolyDemoApp {
	...
	public static void main(String[] args) {
		...
		TerrainGenerator flatGenerator2 = new UnflatTerrainGenerator(noise, colourGen, new StandardIndexGenerator());
		...
		Terrain flatTerrain2 = flatGenerator2.generateTerrain(Configs.TERRAIN_SIZE);
		...
		while (!engine.getWindow().isCloseRequested()) {
			...
			if (Keyboard.isKeyDown(Keyboard.KEY_1)) {
				...
				engine.render(flatTerrain2, camera, light);//old normal terrain
			} else {
